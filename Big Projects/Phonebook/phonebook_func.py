...
def center_window(self, w, h):
    ...
    # Calculates the center of the screen based on demensions 
...
def create_db(self): # Will create the database if one doesn't already exist
...
def first_run(self): # Adds info to DB incase ther isn't any already
    conn = sqlite3.connect('db_phonebook.db')
    with conn:
        cur = conn.cursor()
...
def count_records(cur):
    ...
    count = cur.fetchone()[0] # Gets the first key from each function and saves it as a count
    return cur,count
